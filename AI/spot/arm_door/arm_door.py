...
sys.path.insert(0, '/home/marques/1_Delivery/Alerion/boston-dynamics-spot/IA/neuron/')
...
class RequestManager:
    ...
    def _on_mouse(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            if not self.handle_position_side_by_side:
                ...
                print("***", self.handle_position_side_by_side, "**")
    ...
    def get_user_input_handle_and_hinge2(self, hdl):
        """Open window showing the side by side fisheye images with on-screen prompts for user."""

        # [(560, 183, [(255, 0, 0), (0, 255, 0), (0, 255, 255), (255, 255, 0)]), (364, 183, [(255, 0, 0), (0, 255, 0), (0, 255, 255), (255, 255, 0)])]
        self.handle_position_side_by_side = (hdl[0][0], hdl[0][1])
        _draw_text_on_image(self.side_by_side, "Click handle.")
        cv2.imshow(self.window_name, self.side_by_side)
        cv2.setMouseCallback(self.window_name, self._on_mouse)
        while not self.user_input_set():
...
def choose_open_door(event, x, y, flags, param):
    global openD
    if event == cv2.EVENT_LBUTTONDOWN:
        openD = True
...
def execute_open_door(robot, options):
    ...
    while True:
        ...
        images = request_manager.side_by_side
        cv2.imwrite('./images/image.png', images)
        img, hdl = run(source="./images/image.png", weights="../../neuron/runs/train/handle_det_test4/weights/best.pt", conf_thres=0.25, name="handel_det_test")
        _draw_text_on_image(img, "Choose the door to open")
        cv2.imshow(window_name, img)
        if hdl:
            cv2.setMouseCallback(window_name, choose_open_door)
            # request_manager.get_user_input_handle_and_hinge()
            if openD:
                request_manager.get_user_input_handle_and_hinge2(hdl)
