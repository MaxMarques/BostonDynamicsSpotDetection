...
def main(argv):
    ...
    while True:
        if options.image_sources:
            ...
            for (source, image) in zip(options.image_sources, image_responses):
                ...
                image_saved_path = image_saved_path.replace(
                    "/", '')
                if image_saved_path == "frontright_fisheye_image":
                    cv2.imwrite("./testRight/" + image_saved_path + extension, img)
                elif image_saved_path == "frontleft_fisheye_image":
                    cv2.imwrite("./testLeft/" + image_saved_path + extension, img)
                av = [argv[0]]
                run(source="testLeft", weights="../../neuron/runs/train/handle_det_test4/weights/best.pt", conf_thres=0.25, name="handel_det_test")
                run(source="testRight", weights="../../neuron/runs/train/handle_det_test4/weights/best.pt", conf_thres=0.25, name="handel_det_test")
